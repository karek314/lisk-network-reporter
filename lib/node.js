...
Node.prototype.askLisk = function(method, endpoint, action, misc, self) {
	...
	var req = http.request(options, function(res) {
		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
       		...
       		if (action == 'check') {
       			var state = jsonObject["data"]["loaded"];
       			isConnected = state;
       		} else if (action == 'lastblock') {
       			var block = jsonObject["data"][0];
       			self.validateLatestBlock(null, block, misc, self);
       		} else if (action == 'nodeinfo') {
       			jsonObject = jsonObject["data"];
       			var NodeInfoString = "Lisk " + jsonObject["version"] + jsonObject["commit"].substring(0,8) + " Build:(" +jsonObject["build"].substring(0, jsonObject["build"].length - 1)+")";
       			NodeInfoString = NodeInfoString.replace("-",".");
       			NodeInfoString = NodeInfoString.replace(/\s/g,".");
       			console.info('Got info');
				console.info(NodeInfoString);
       			self.info.coinbase = NodeInfoString;
				self.info.node = NodeInfoString;
				self.info.net = process.env.NETWORK_MODE;
				if (process.env.IS_FORGING == 'yes') {
 					self.stats.hashrate = 1;
 					self.stats.mining = true;
 					console.info('==>', 'Forging: ',self.stats.hashrate)
 				} else {
 					self.stats.hashrate = 0;
 					self.stats.mining = false;
 					console.info('==>', 'Not Forging')
 				}
				...
				var pending = jsonObject["data"]["transactions"]["unprocessed"];
				var results = {};
				results.end = _.now();
				...
				if (syncing == true) {
					console.info("SYNC STARTED:", syncing);
					self.stats.syncing = true;
					if(self._lastBlock !== current_block) {
						self._latestQueue.push(current_block);
					}
					console.info("SYNC UPDATE:", syncing);
				} else {
				...
				var peer_count = jsonObject["meta"]["count"];
...
Node.prototype.startConnection = function() {
	console.info('Starting connection to Lisk Node');
...
Node.prototype.checkConnection = function() {
	var self = this;
	self.askLisk('GET','/api/node/status','check', '', self);
	if(isConnected) {
...
Node.prototype.startSocketConnection = function() {
	if( !this._socket ){
		...
		socket = new Socket( process.env.WS_SERVER || 'ws://liskstats.net:3000');
		this.setupSockets();
	}
}
...
Node.prototype.getInfo = function() {
	...
	self.askLisk('GET','/api/node/constants','nodeinfo', '', self);
	return true;
}
...
Node.prototype.getLatestBlock = function () {
	...
	self.askLisk('GET','/api/blocks?limit=1','lastblock', timeString, self);
}
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		...
		self.getPending();
		self.askLisk('GET','/api/peers','peers', '', self);
		self.getLatestBlock();
		self.getInfo();
...
Node.prototype.getPending = function() {
	...
	self.askLisk('GET','/api/node/status','pendingtx', now, self);
	return false;	
}
...
Node.prototype.sendBlockUpdate = function() {
	...
	STATS_ITERATION--;
	if (STATS_ITERATION < 1) {
		//update node info after 60 blocks
		this.emit('hello', {id: this.id,info: this.info,secret: WS_SECRET});
		STATS_ITERATION = 60;
	}
}
...
Node.prototype.setWatches = function() {
	...
	self.askLisk('GET','/api/node/status','syncing', '', self);
}
